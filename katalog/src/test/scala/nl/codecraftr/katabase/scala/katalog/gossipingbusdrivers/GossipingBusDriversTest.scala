...
class GossipingBusDriversTest extends AnyFlatSpec with Matchers:
  "gossip" should "return amount of minutes given all gossips exchanged during workday" in:
    val input =
      Source.fromResource("all-exchanged.txt").getLines().mkString("\n")

    GossipingBusDrivers.simulate(input) shouldBe "5"

  it should "return 'never' given gossips are not all exchanged during workday" in:
    val input =
